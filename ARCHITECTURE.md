...
The raw frame is slice of the stack page, it is managed through a `GcFrame`. A `GcFrame` has a number of slots, a number of rooted values, and a capacity. Unlike a frame in the Julia C API, a `GcFrame` in jlrs can grow its number of slots dynamically up to its capacity. It cannot span multiple `StackPage`s. Most of a `GcFrame`s functionality can be found in the `Frame` trait. Neither `GcFrame` nor `Frame` offer a public method the create a new `GcFrame`, rather methods like `frame`, `value_scope`, and `call_scope` are available which take a closure. These methods create and push a new `GcFrame` before calling the closure with a mutable reference to that new frame, when the frame is dropped it's popped from the stack. If possible, the raw frame backing this new frame is acquired from the current frame's remaining capacity. Schematically, contents of the stack page looks like
