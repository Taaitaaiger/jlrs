...
/// Generate an initialization function that can be used by `Jlrs.Wrap.@wrapmodule`.
///
/// The following things can be exported to Julia using this macro:
///
///  - `unsafe extern "C" fn`s where all arguments implement `CCallArg` and the return type
///    implements `CCallReturn`.
///
///  - Types that implement `ForeignType`.
///
...
///    account for the possibility that the data is currently inaccessible.
///
...
///     // with the same name it has in Rust.
...
///
///     // Exposes the struct `MyType` as `MyForeignType`. `MyType` must implement `ForeignType`.
