...
function print_help() {
    ...
    echo ""
    echo -e "\033[1m      Version                  Default path${spacing}Override\033[0m"
    echo "  Linux 64-bit 1.9         $HOME/julia-1.9.0-beta3        JULIA_1_9_DIR"
    echo "  Linux 32-bit 1.9         $HOME/julia-1.9.0-beta3-32     JULIA_1_9_DIR_32"
    echo "  Windows 64-bit 1.9       $HOME/julia-1.9.0-beta3-win    JULIA_1_9_DIR_WIN"
    echo ""
    echo ""
...
if [ "${BETA}" = "y" -o "${ALL}" = "y" ]; then
    if [ -z "$JULIA_1_9_DIR" ]; then
        JULIA_1_9_DIR=${HOME}/julia-1.9.0-beta3
    fi
    ...
    if [ -z "$JULIA_1_9_DIR_32" ]; then
        JULIA_1_9_DIR_32=${HOME}/julia-1.9.0-beta3-32
    fi
    ...
    if [ -z "$JULIA_1_9_DIR_WIN" ]; then
        JULIA_1_9_DIR_WIN=${HOME}/julia-1.9.0-beta3-win
    fi
