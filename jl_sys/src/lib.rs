...
#![cfg_attr(feature = "c-unwind", feature(c_unwind))]
...
cfg_if! {
    if #[cfg(feature = "c-unwind")] {
        pub mod bindings_unwind;
        pub use bindings_unwind as bindings;
        pub use bindings::*;

        #[cfg(feature = "julia-1-6")]
        pub const jl_init: unsafe extern "C-unwind" fn() = jl_init__threading;

        #[cfg(feature = "julia-1-6")]
        pub const jl_init_with_image: unsafe extern "C-unwind" fn(*const c_char, *const c_char) =
...
pub unsafe fn jl_astaggedvalue(v: *mut jl_value_t) -> *mut jl_taggedvalue_t {
    let sz = size_of::<jl_taggedvalue_t>() / size_of::<usize>();
    v.cast::<usize>().sub(sz).cast::<jl_taggedvalue_t>()
}
