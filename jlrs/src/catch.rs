            ...
            julia
                .scope(|_global, mut frame| unsafe {
...

                    {
                        ...
                        let mut callback =
                            |frame_slice: &mut FrameSlice, result: &mut MaybeUninit<()>| {
                                arr.grow_begin_unchecked(&mut *frame_slice, 5);
                                result.write(());
                            };

                        let res = catch_exceptions_with_slots(&mut frame, 0, &mut callback);
                        assert!(res.is_ok());
                        let res = res.unwrap();
