...
//! fuction with pending drops can prevent that data from being dropped at best, and is terribly
//! UB at worst.
//!
...
use crate::{
    data::{layout::foreign::ForeignType, managed::value::Value},
...
/// the data is transfered from Rust to Julia. Unlike other managed types, `WithParachute` doesn't
/// implement [`Managed`] but behaves like a mutable reference to the protected data: it
/// implements `Deref` and `DerefMut` to allow using protected data as if it were a (mutable)
/// reference to the original data. The  parachute can be removed by calling
/// `WithParachute::remove_parachute` to regain ownership.
///
...
pub trait AttachParachute: 'static + Sized + Sync {
    /// Attach a parachute to the data.
    ///
    ...
    fn attach_parachute<'scope, T: Target<'scope>>(self, target: T) -> WithParachute<'scope, Self> {
        ...
        unsafe {
            let mut ptr: NonNull<Option<Self>> = std::mem::transmute_copy(&data);
