...
#[inline]
unsafe extern "C-unwind" fn trampoline<'frame, F: FnMut() -> T, T>(
    func: &mut F,
...
fn trampoline_for<'frame, F: FnMut() -> T, T>(
    _: &mut F,
) -> Option<unsafe extern "C-unwind" fn(*mut c_void, *mut c_void) -> jlrs_catch_t> {
    unsafe {
        std::mem::transmute::<
            Option<unsafe extern "C-unwind" fn(&mut F, &mut MaybeUninit<T>) -> jlrs_catch_t>,
            Option<unsafe extern "C-unwind" fn(*mut c_void, *mut c_void) -> jlrs_catch_t>,
        >(Some(trampoline::<F, T>))
    }
}
