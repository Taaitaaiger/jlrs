//! jlrs provides a reasonably safe interface to the Julia C API that lets embed Julia in Rust
//! applications. Currently this crate is only tested on Linux in combination with Julia 1.6 and
...
//! function calls like `sqrt(2.0)`, but can't take any arguments. It's most important use-case is
...
//! [`IntoJulia`]. This trait is implemented by primitive types like `i8` and `char`.
//!
//!To create a `Value` directly, without evaluating Julia code, other methods
