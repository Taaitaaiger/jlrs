...
unsafe impl ValidLayout for u128 {
    #[inline]
    fn valid_layout(v: Value) -> bool {
        if v.is::<DataType>() {
            let dt = unsafe { v.cast_unchecked::<DataType>() };
    ...
    #[inline]
    fn type_object<'target, Tgt: Target<'target>>(target: &Tgt) -> Value<'target, 'static> {
        Self::base_type(target).unwrap()
    }

    const IS_REF: bool = false;
}

unsafe impl ValidLayout for i128 {
    #[inline]
    fn valid_layout(v: Value) -> bool {
        if v.is::<DataType>() {
            let dt = unsafe { v.cast_unchecked::<DataType>() };
...
unsafe impl ValidField for u128 {
    #[inline]
...
unsafe impl ValidField for i128 {
    #[inline]
    fn valid_field(v: Value) -> bool {
        if v.is::<DataType>() {
            let dt = unsafe { v.cast_unchecked::<DataType>() };
