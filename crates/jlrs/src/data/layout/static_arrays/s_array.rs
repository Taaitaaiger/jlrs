    ...
    IntoJulia for &SArray<T, D, N, R>
{
    ...
    {
        unsafe {
            SArray::<T, D, N, R>::construct_type(&target)
                .as_managed()
                .cast::<DataType>()
                .unwrap()
                .root(target)
        ...
        Tgt: Target<'scope>,
    {
        unsafe {
            ...
            container
                .cast::<MaybeUninit<SArray<T, D, N, R>>>()
...
unsafe impl<T: ConstructType, D: Dims<R>, const N: usize, const R: usize> ConstructType
    for SArray<T, D, N, R>
{
    type Static = SArray<T::Static, D, N, R>;
        ...
        Tgt: Target<'target>,
    {
        target.with_local_scope::<_, 6>(|target, mut frame| unsafe {
            ...
            let s_array = Self::base_type(&mut frame).unwrap();
    ...
    where
        Tgt: Target<'target>,
    {
        let var = inline_static_ref!(STATIC, Value, "StaticArrays.SArray", target);
        Some(var)
    }
}

unsafe impl<T: IsBits, D: Dims<R>, const N: usize, const R: usize> IsBits for SArray<T, D, N, R> {}
...
unsafe impl<T: ConstructType, D: Dims<R>, const N: usize, const R: usize> CCallArg
    for &SArray<T, D, N, R>
{
    type CCallArgType = RefTypeConstructor<SArray<T, D, N, R>>;
    type FunctionArgType = SArray<T, D, N, R>;
}
