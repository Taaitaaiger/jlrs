...
unsafe impl<T: ConstructType, const N: usize> Typecheck for MVector<T, N> {
...
unsafe impl<T: ConstructType + Clone, const N: usize> Unbox for MVector<T, N> {
    type Output = Self;
}

unsafe impl<T: 'static + ConstructType + Clone, const N: usize> IntoJulia for MVector<T, N> {
...
unsafe impl<'a, T: 'static + ConstructType + Clone, const N: usize> IntoJulia for &MVector<T, N> {
    ...
    {
        unsafe {
            MVector::<T, N>::construct_type(&target)
                .as_managed()
                .cast::<DataType>()
                .unwrap()
                .root(target)
...
unsafe impl<T: ConstructType, const N: usize> ConstructType for MVector<T, N> {
    type Static = MVector<T::Static, N>;
        ...
        Tgt: Target<'target>,
    {
        target.with_local_scope::<_, 4>(|target, mut frame| unsafe {
            ...
            assert!(m_vector.is::<UnionAll>());
            let m_vector_ua = m_vector.cast_unchecked::<UnionAll>();
    ...
    where
        Tgt: Target<'target>,
    {
        let var = inline_static_ref!(STATIC, Value, "StaticArrays.MVector", target);
        Some(var)
    }
}

unsafe impl<T: IsBits, const N: usize> IsBits for MVector<T, N> {}

unsafe impl<T: ConstructType, const N: usize> CCallArg for &MVector<T, N> {
    type CCallArgType = RefTypeConstructor<MVector<T, N>>;
    type FunctionArgType = MVector<T, N>;
}

unsafe impl<T: ConstructType, const N: usize> CCallArg for &mut MVector<T, N> {
    type CCallArgType = RefTypeConstructor<MVector<T, N>>;
    type FunctionArgType = MVector<T, N>;
}

unsafe impl<T: ConstructType, const N: usize> CCallReturn for MVector<T, N> {
