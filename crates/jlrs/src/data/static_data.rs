...
use crate::{
    data::{
        ...
        managed::{Managed, array::TypedVector, module::Module, value::ValueUnbound},
    },
...
pub(crate) static LOADING_PACKAGE_DEPS: AtomicPtr<jl_array_t> = AtomicPtr::new(null_mut());

// Safety: must only be called by initialization functions generated by `julia_module!`.
#[doc(hidden)]
pub unsafe fn set_loading_package(module: Option<(Module, TypedVector<Module>)>) {
    match module {
        Some((module, deps)) => {
            LOADING_PACKAGE.store(module.as_weak().ptr().as_ptr(), Ordering::Relaxed);
            LOADING_PACKAGE_DEPS.store(deps.as_weak().ptr().as_ptr(), Ordering::Relaxed);
        }
        None => {
            LOADING_PACKAGE.store(null_mut(), Ordering::Relaxed);
            LOADING_PACKAGE_DEPS.store(null_mut(), Ordering::Relaxed);
        }
    }
}
        ...
        match item_name {
            ...
            item => {
                ...
                if !ptr.is_null() {
                    ...
                    // Is a dependency of the current package requested?
                    let ptr = LOADING_PACKAGE_DEPS.load(Ordering::Relaxed);
                    if !ptr.is_null() {
                        let deps = TypedVector::<Module>::wrap_non_null(
                            NonNull::new_unchecked(ptr),
                            Private,
                        );
                        let acc = deps.managed_data_unchecked::<Module>();
                        for dep in acc.into_slice() {
                            let m = dep.load(Ordering::Relaxed).unwrap().as_managed();
                            if m.name().as_bytes() == item.as_bytes() {
                                if let Ok(module) = m.leak().as_value().cast::<T>() {
                                    return Ok(module);
                                }
