...
pub(crate) fn top_module<'target, Tgt: Target<'target>>(
    target: &Tgt,
    module_name: &str,
) -> JlrsResult<Module<'target>> {
    match module_name {
        "Main" => Ok(Module::main(target)),
        "Base" => Ok(Module::base(target)),
        "Core" => Ok(Module::core(target)),
        "JlrsCore" => Ok(Module::jlrs_core(target)),
        pkg => {
            // Try to find this module in the list of loaded packages JlrsCore maintains internally
            match Module::package_root_module(target, pkg) {
                Some(module) => Ok(module),
                None => unsafe {
                    // `LOADING_PACKAGE` is set when an init function generated by `julia_module!`
                    // is called.
                    let ptr = LOADING_PACKAGE.load(Ordering::Relaxed);
                    let m = match NonNull::new(ptr) {
                        Some(ptr) => {
                            // A package is currently being loaded
                            let current = WeakModule::wrap(ptr).as_managed();
                            let name = current.name().as_str()?;
                            if name == pkg {
                                // Names match, the currently loading package is the one we need
                                current
                            } else {
                                // Search in `Main`
                                Module::main(target).submodule(target, pkg)?.as_managed()
                            }
                        }
                        // Search in `Main`
                        None => Module::main(target).submodule(target, pkg)?.as_managed(),
                    };
                    Ok(m)
                },
            }
        ...
        Tgt: Target<'target>,
    {
        unsafe {
            ...
            let global = self.global.get_or_init(|| {
                ...
                let mut module = top_module(target, split_path[0]).unwrap();
        ...
        Tgt: Target<'target>,
    {
        unsafe {
            ...
            let mut module = match split_path[0] {
                ...
                pkg => match Module::package_root_module(target, pkg) {
                    Some(module) => module,
                    None => {
                        let ptr = LOADING_PACKAGE.load(Ordering::Relaxed);
                        let ptr = NonNull::new(ptr).unwrap();
                        let module = WeakModule::wrap(ptr);
                        module.as_managed()
                    }
                },
            };
